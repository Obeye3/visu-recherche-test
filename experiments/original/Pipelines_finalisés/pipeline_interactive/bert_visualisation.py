...
def prepare_data():
    ...
    print(f"Nombre de documents : {len(documents)}")
    print("Exemple de document :", documents[0])
    print("Documents vides :", [doc for doc in documents if len(doc.strip()) == 0])
...
def create_keywords_plot(topic_id):
    ...
    pio.write_html(fig, "bertopic_visualisation.html", auto_open=True)
    return fig
