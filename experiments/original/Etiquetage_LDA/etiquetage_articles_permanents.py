...
def main(num_topics):
    ...
    for idx, m in enumerate(matches):
        ...
        tokens = [tok for tok in simple_preprocess(full_text) if tok not in STOP_ALL and len(tok) > 2]
...
# 1. Récupération des titres et mots-clés
    for i in range(num_topics):
        topic_titles = [info['title'] for hal_id, info in filtered[i].items() if 'title' in info]
        titles_raw.append(topic_titles)
...
# 2. Extraction des bigrammes
    bigrams_freq = [extract_ngrams_list(text, n=2, min_freq=1) for text in titles_raw]
    bigrams_freq = [remove_stopword_bigrams(bigram) for bigram in bigrams_freq]
...
# Exemple d'affichage des top keywords par topic :
    for i, keywords in enumerate(sorted_keywords_by_topic):
        print(f"Topic {i+1}:")
        for bigrams in list(bigrams_freq[i].items())[:3]: # Affichage top bigrams
